...
public class B738 extends SeatType {
	...
	String[][] str = new String[6][31]; //좌석번호 담은 배열 (1A~31F)
	int[][] seatCount = new int[6][31]; //좌석 reserved or empty
	...
	public B738() {
		...
		for(int i = 0; i < seatCount.length; i++) {
			...
			for(int j = 0; j < seatCount[i].length; j++) {
				...
				if(rVO.getFlights().equals("B738") && rVO.getReserved_seat_num().equals(str[i][j])) {
