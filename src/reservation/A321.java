...
public class A321 extends SeatType {
	...
	String[][] str = new String[6][43]; //좌석번호 담은 배열 (1A~43F)
	int[][] seatCount = new int[6][43]; //좌석 reserved or empty
	...
	public A321() {
		...
		for(int i = 0; i < seatCount.length; i++) {
			...
			for(int j = 0; j < seatCount[i].length; j++) {
				...
				if(rVO.getFlights().equals("A321") && rVO.getReserved_seat_num().equals(str[i][j])) {
